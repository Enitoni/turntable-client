	...
	return (
		<div className="flex items-center gap-4">
			<div className="border-2 rounded-md border-white/10 overflow-clip">
				<a target="_blank" href={item.track.canonical} rel="noopener noreferrer"><TrackCover track={item.track} className="size-10" /></a>
			</div>
			<div className="flex flex-col gap-1">
				<div className="flex flex-col">
					<span className="font-medium line-clamp-1 text-ellipsis">
						<a target="_blank" href={item.track.canonical} rel="noopener noreferrer">{item.track.artist} - {item.track.title}</a>
					</span>
